                ...
                columns: [
                    ...
                    render: function(data, type){
                        if(type === 'display'){
                            if(data == 'RSV') {
                              ...
                              return '<span class="badge badge-warning">CANCEL</span>';
                            } else {
                    ...
                    }},
                    {
                        ...
                        render: function(data, type, row) {
                          ...
                          @can(session('menuId') . '_update')                          
                          let delete_disabled = (row.sell_status == 'CANCEL' || row.sell_status == 'SELL' ? 'd-none' : '');                          
                        
                            btn += 
                                  '<button onclick="return toggleState('+ data +');" class="'+delete_disabled+' btn btn-' + (row.sell_status ? "danger":"primary") +'" data-toggle="tooltip" title="'+ (row.sell_status ? "Disable" : "Enable") +'"><i class="fa '+ (row.sell_status ? "fa-ban" : "fa-check") +'"></i></button>'
                          @endcan
            ...
            success: function (res) {
              ...
              if (res.res) {
                ...
                res.result.detail.forEach(val => {  
                  let desc = val.selldtl_desc ? val.selldtl_desc : '';
                                  
                  let row = '<tr>' +
                    '<td>' + val.selldtl_order + '</td>' +
                    '<td>' + val.selldtl_transno + '</td>' +
                    '<td>' + val.selldtl_asset_name + '</td>' +
                    '<td>' + desc + '</td>' +
                    '<td>'+ AutoNumeric.format(val.selldtl_price, {
                      currencySymbol: 'Rp. ',
                      allowDecimalPadding: 'floats'
                    }) + '</td>' +
                  '</tr>';
