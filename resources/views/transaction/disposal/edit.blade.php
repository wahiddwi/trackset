    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('disposal.update', $disposal->id) }}" method="POST">
                ...
                {{-- <input type="hidden" name="dis_site" id="dis_site" value="{{ $disposal->site->si_site }}"> --}}

                <div class="row">
                  <x-adminlte-input name="dis_transno" label="Disposal No." placeholder="AUTO GENERATED" igroup-size="md" fgroup-class="col-md-6" error-key="dis_transno" value="{{ $disposal->dis_transno }}" disabled/>
                  {{-- <x-adminlte-input name="dis_transdate" label="Tgl. Transaksi" label-class="must-fill" igroup-size="md" fgroup-class="col-md-6" error-key="dis_transdate" type="date" value="{{ $disposal->dis_transdate }}" /> --}}
                  @php
                    $config = [
                        'format' => 'DD MMM YYYY',
                ...
                <div class="row">
                  <x-adminlte-textarea name="dis_desc" label="Keterangan" label-class="must-fill" igroup-size="md" fgroup-class="col-md-12" error-key="dis_desc" rows=4>{{ $disposal->dis_desc }}</x-adminlte-textarea>
                </div>
                <hr>
                <div class="row">
...
@section('js')
    ...
    <script>
        $(document).ready( function () {
          assetNo = [];
          @foreach($disposal->detail as $dtl)
            editRow({{ $dtl->disdtl_order }}, @php echo json_encode($dtl->toArray()); @endphp)
          @endforeach
          ...
          let tr = 
            `<tr data-row="${row}" class="${row}" id="${row}">
              <td>
                <div class="form-group">
                  <div class="input-group">
                    <input class="form-control inv_transno" id="inv_transno" value="${data.disdtl_asset_transno}" disabled/>
                    <input type="hidden" name="disdtl_asset_transno[]" value="${data.disdtl_asset_transno}">
                    <input type="hidden" name="disdtl_asset_site[]" value="${data.disdtl_asset_site}">
                    <input type="hidden" name="disdtl_order[]" value="${data.disdtl_order}">
                  </div>
                </div>
              </td>
              <td>
                <div class="form-group">
                  <div class="input-group">
                    <input class="form-control inv_name" id="inv_name" value="${data.disdtl_asset_name}" disabled/>
                    <input type="hidden" name="disdtl_asset_name[]" value="${data.disdtl_asset_name}">
                  </div>
                </div>
              </td>
              <td>
                <div class="form-group">
                  <div class="input-group">
                    <input class="form-control remark" id="remark" value="${data.disdtl_desc?data.disdtl_desc:''}"/>
                    <input type="hidden" name="disdtl_desc[]" value="${data.disdtl_desc?data.disdtl_desc:''}">
                  </div>
                </div>
              </td>
              <td>
