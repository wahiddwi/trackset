...
<head>
    ...
    <title>RCV - {{ $receive->transfer->trf_id }}</title>
    <style type="text/css">
        @page {
            size: 21cm 14cm;
            margin: 0cm 0cm;
...
<body>
    <div class="header">
        ...
        <h6 class="right-text"><strong>{{ $receive->transfer->trf_id }}</strong></h6>
        <hr>
    </div>
    ...
    <table id="purchaseTable" style="margin-top: 20px;" border="0" cellspacing="0" cellpadding="0" width="100%">
        <tr>
            ...
            <th style="width: 1%;"></th>
    {{-- @dd($receive) --}}
            <td style="width: 30%;">{{ $receive->transfer->siteFrom->si_name }}</td>
            <th style="width: 15%">Lokasi Asal</th>
            <th style="width: 1%;"></th>
            <td style="width: 30%;">{{ $receive->transfer->locFrom->loc_name }}</td>
        </tr>
        <tr>
            <th style="width: 15%">Cabang Tujuan</th>
            <th style="width: 1%;"></th>
            <td style="width: 30%;">{{ $receive->transfer->siteTo->si_name }}</td>
            <th style="width: 15%">Lokasi Tujuan</th>
            <th style="width: 1%;"></th>
            <td style="width: 30%;">{{ $receive->transfer->locTo->loc_name }}</td>
        </tr>
        <tr>
            <th style="width: 15%">PIC Asal</th>
            <th style="width: 1%;"></th>
            <td style="width: 30%;">{{ $receive->transfer->userFrom != null && $detail->transfer->trf_pic_from == $detail->transfer->userFrom->usr_nik ? $detail->transfer->userFrom->usr_name : $detail->transfer->siteFrom->si_name }}</td>
